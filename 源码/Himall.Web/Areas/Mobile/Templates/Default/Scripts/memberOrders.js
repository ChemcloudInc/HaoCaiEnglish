                ...
                $.each(result, function (i, item) {
                    if (!(status == 5 && item.commentCount != 0)) {
                        ...
                        $.each(item.itemInfo, function (j, orderItem) {
                            html += '<div class="order-goods clearfix"><a href="/' + areaName + '/Order/Detail?id=' + item.id + '"><img src="' + orderItem.image + '" /><p>' + orderItem.productName + '</p>';
                        ...
                        html += '<p class="order-text"><a href="/' + areaName + '/Order/Detail?id=' + item.id + '"><span>共 ' + item.productCount + ' 件商品</span> <span>实付： ¥' + item.orderTotalAmount + '</span></a></p>';
